"""
Closed-loop control flow aligned with repair/brain_api_example.SimpleBrain.run_brain_loop:

  get images -> analyze_vision -> decide_action -> set_velocity_command -> rate limit

The body is abstracted as RobotWalkBody so hardware and Sapien can be swapped.
"""
...
class TaskPlanningLoop:
    """Runs the same tick loop as brain_api_example; policy produces velocity commands."""
    ...
    def run_closed_loop(
        ...
        """Same structure as SimpleBrain.run_brain_loop; max_ticks exits early for sim demos."""
        print(
            "[task_planning] Closed loop started "
        ...
        try:
            while self.running:
                ...
                if images:
                    ...
                    print(
                        f"[task_planning] tick={tick} brightness={vision_info.get('brightness', 0):.1f} "
                    ...
                    print("[task_planning] No vision; zero velocity.")
            ...
            print("\n[task_planning] KeyboardInterrupt")
        finally:
            self.body.set_velocity_command(0.0, 0.0, 0.0, 0.8)
            self.running = False
            self.body.shutdown()
            print("[task_planning] Loop stopped.")
