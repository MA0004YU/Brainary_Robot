"""Real robot / Isaac+DDS: delegates to repair/g1_walk_controller.WalkController (same backend as brain_api_example)."""
...
class RepairWalkBody:
    """Same API shape as using WalkController directly inside SimpleBrain."""
    ...
    def start(self) -> None:
        ...
        if r not in sys.path:
            sys.path.insert(0, r)
        from g1_walk_controller import WalkController  # noqa: WPS433

        self._controller = WalkController()
        self._thread = threading.Thread(target=self._controller.run, daemon=True)
        self._thread.start()
        print("[task_planning] RepairWalkBody: waiting for DDS / controller...")
