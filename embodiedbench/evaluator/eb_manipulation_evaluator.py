...
class EB_ManipulationEvaluator():
    def __init__(self, config):
        ...
        self.env = None
        self.planner = None
        self.arch_memory = self.config.get('arch_memory_instance')
        if self.arch_memory is None and self.config.get('use_architectural_memory', 0):
    ...
    def evaluate(self):
        ...
        while self.env._current_episode_num < self.env.number_of_episodes:
            ...
            if self.arch_memory is not None:
                self.arch_memory.reset_episode()
                self.arch_memory.begin_task(user_instruction, self.env.current_task_variation)
                # Perception port: colleagues replace NullPerception; image paths passed as meta until then.
                try:
                    self.arch_memory.run_perception({"image_paths": list(img_path_list), "obs": obs})
                except Exception as e:
                    logger.warning("Architectural perception hook failed (expected until implemented): %s", e)
            ...
            while not done:
                ...
                if self.arch_memory is not None:
                    self.arch_memory.record_planner_reasoning(reasoning if isinstance(reasoning, str) else str(reasoning))
                if len(action) == 0:
                    ...
                    for action_single in action[:min(self.env._max_episode_steps - self.env._current_step, len(action))]:
                        ...
                        if self.arch_memory is not None:
                            self.arch_memory.record_discrete_action(action_single)
                            self.arch_memory.record_environment_feedback(info)
                            try:
                                self.arch_memory.run_monitor({"obs": obs, "paths": img_path_list})
                            except Exception as e:
                                logger.warning("Monitor hook failed (stub): %s", e)
                            try:
                                self.arch_memory.propose_simulated_outcome(action_single, horizon=1)
                            except Exception as e:
                                logger.warning("Simulation hook failed (stub): %s", e)
                        img_path_list = self.env.save_image(camera_views)
            ...
            if self.arch_memory is not None:
                self.arch_memory.end_episode(
                    success=bool(info.get("task_success", 0)),
                    extra={"planner_steps": self.planner.planner_steps},
                )
                mem_path = os.path.join(self.env.log_path, 'results', f"memory_episode_{self.env._current_episode_num}.json")
                try:
                    with open(mem_path, 'w', encoding='utf-8') as mf:
                        json.dump(self.arch_memory.snapshot(), mf, ensure_ascii=False, indent=2)
                except Exception as e:
                    logger.warning("Could not write memory snapshot: %s", e)
