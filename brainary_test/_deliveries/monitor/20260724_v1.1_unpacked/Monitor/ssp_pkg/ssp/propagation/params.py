...
class PropagationParams(BaseModel):
    ...
    # ADR-027: semantic-uncertainty (§3.4 u_semantic) assigned to a scene-intrinsic
    # factor whose physical precondition is UNDECIDABLE (unknown / missing state).
    # Routed to the `uncertain` bucket with risk.uncertainty.semantic set to this
    # value, instead of fabricating activation or silently dropping the factor.
    u_semantic_unresolved: Annotated[float, Field(ge=0.0, le=1.0)] = 0.5
